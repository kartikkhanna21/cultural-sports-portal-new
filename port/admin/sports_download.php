...
$sql="SELECT DISTINCT event_level from `sports_docs` WHERE
    ...
    while($row = mysqli_fetch_array($setRec)) {
     ...
     {$setData1 = '';
        ...
        $i=1;
        $sql="SELECT event_name, event_org,event_level,name,awards_participate from `sports_docs`
...
echo "\n\t\tOnly Participated = ".$count_participate;
    ...
    $file_folder = "../Docs/Sports_docs/";
    $zipname = $yea.'_Sports_docs.zip';
    $zip = new ZipArchive;
    ...
    $file_folder = "../Photo/Sports_photo/";
    $zipname = $yea.'_Sports_photos.zip';
    $zip = new ZipArchive;
    $zip->open($zipname, ZipArchive::CREATE);
    foreach ($docs as $file) {
        $path=$file_folder.$file;
